...
const CategoryActivities = () => {
  ...
  // STATES
  const [categoryTreeActivities, setCategoryTreeActivities] = useState([]);
  ...
  // Table Data
  const [filteredCategoryTreeActivities, setFilteredCategoryTreeActivities] =
    useState([]);
  ...
  const fetchCategoryTreeActivities = async () => {
    try {
      setTableLoading(true);
      const response = await axios.get(
        `${apiUrls.urlPrefix}/activity-window/categ-tree-activities`,
        requestHeader.json
      );
      if (response?.data) {
        const resData = response?.data;
        setCategoryTreeActivities(resData?.result);
        setFilteredCategoryTreeActivities(resData?.result);
  ...
  const resetFilters = () => {
    ...
    setFilteredCategoryTreeActivities(categoryTreeActivities);
  };

  // Filter help desk data
  useEffect(() => {
    const filteredData = categoryTreeActivities.filter((row) => {
    ...
    setFilteredCategoryTreeActivities(filteredData);
  }, [section, status, startDate, endDate, categoryTreeActivities]);
    ...
    <div>
      ...
      >
        ...
        <table className="table table-striped" style={{ fontSize: rowFont }}>
          ...
          >
            <tr>
              <th style={defaultStyleHead}>L1 Category</th>
              <th style={defaultStyleHead}>L2 Category</th>
              <th style={defaultStyleHead}>L3 Category</th>
          ...
          <tbody>
            {filteredCategoryTreeActivities.map((row) => (
              <tr key={row.id} style={{ borderBottom: "1px solid #ddd" }}>
                <td style={defaultStyles}>{row.L1Category || "N/A"}</td>
                <td style={defaultStyles}>{row.L2Category || "N/A"}</td>
                <td style={defaultStyles}>{row.L3Category || "N/A"}</td>
                <td style={defaultStyles}>{row.L4Category || "N/A"}</td>
                ...
                <td style={defaultStyles}>
                  <span
                    onClick={() => {
                      navigate(
                        `/activity-window/categ-activity-details/${row.Id}`
                      );
                    }}
