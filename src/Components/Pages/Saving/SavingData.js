...
const SavingData = () => {
  ...
  const [savingKpiMain, setSavingKpiMain] = useState();
  const [savingData, setSavingData] = useState([]);
  const [savingMain, setSavingMain] = useState([]);
  const pageSize = 1000;
  ...
  const getSavingKpi = async () => {
    await axios
      .get(
        `${
          apiUrls.urlPrefix
        }/saving-kpis?SavingData_Clause=${encodeURIComponent(finalQuery)}`,
        requestHeader.json
      )
      .then((response) => {
        ...
        setSavingKpi(data);
        if (!savingKpiMain) {
          console.log("set saving kpi main");
          setSavingKpiMain(data);
        }
      })
      .catch((error) => {
  ...
  //Get saving Filters
  const getSavingFilters = async () => {
    console.log(finalQuery, "filters");
    await axios
      .get(
        `${
          apiUrls.urlPrefix
        }/saving-filters?SavingData_Clause=${encodeURIComponent(finalQuery)}`,
        requestHeader.json
      )
      .then((response) => {
        const data = response.data.result;
  ...
  // Function to load saving data
  const getSavingData = async (loadCount, page, localFilter) => {
    setLoading(true);
    await axios
      .get(
        `${
          apiUrls.urlPrefix
        }/saving-data?lastId=${loadCount}&pageSize=${page}&SavingdData_Clause=${encodeURIComponent(
          finalQuery
        )}`,
        requestHeader.json
      )
      .then((response) => {
        ...
        handleSavingResponse(data, localFilter);
      })
      .catch((error) => {
        setLoading(false);
        console.log(error.message);
  ...
  //Reponse Function
  function handleSavingResponse(res, localFilter) {
    if (finalQuery === "") {
      if (savingData.length === 0) {
        ...
        let ar = savingData;
        setSavingData([...ar, ...res]);
        setSavingMain([...ar, ...res]);
      }
  ...
  const filterThoughInitialData = (isAll) => {
    ...
    const filteredData = arr.filter((item) => {
      ...
      const matchesCountry =
        country && country !== "All" ? item.CountryCode === country : true;
      const matchesYear =
  ...
  const handleLoadMore = async () => {
    if (savingData.length < pageSize) return; // if data already less than pageSize, don't load more

    gridRef.current.api.showLoadingOverlay(); // Show AG Grid loading overlay

    try {
      if (finalQuery === "") {
        await getSavingData(lastId, pageSize); // Wait for data to be loaded
      } else {
        await getSavingData(lastId, pageSize, savingData); // Wait for data to be loaded
      }
    } catch (error) {
  ...
  const handleApplyFilter = async () => {
    ...
    try {
      ...
      if (localFilter.length !== pageSize || localFilter.length < pageSize) {
        ...
        if (!isAllSelected) {
          await getSavingData(lastId, newPageSize, localFilter); // Wait for data load
        } else {
          await getSavingData(mainId, newPageSize, localFilter); // Wait for data load
          setIsAllSelected(false);
        }
      }

      // Fetch KPIs and filters after the data is loaded
      await getSavingKpi(); // Wait for KPI fetch
      await getSavingFilters(); // Wait for filters fetch
    } catch (error) {
  ...
  const handleExport = async () => {
    ...
    try {
      ...
      const res = await axios.get(
        `${
          apiUrls.urlPrefix
        }/saving-export?Rows=${exportRows}&SavingData_Clause=${encodeURIComponent(
          finalQuery
        )}`,
      ...
      link.href = url;
      link.setAttribute("download", "SavingData.xlsx");
  ...
  return (
    <div>
      ...
      <div className="global-kpi-container">
        ...
        <div className="global-kpi-box">
          <p className="global-kpi-num">
            {savingKpi?.transactionCount
              ? formatCompactNumber(savingKpi.transactionCount)
              : 0}
        ...
        <div className="global-kpi-box">
          <p className="global-kpi-num">
            {savingKpi?.countryCount
              ? formatCompactNumber(savingKpi.countryCount)
              : 0}
        ...
        <div className="global-kpi-box">
          <p className="global-kpi-num">
            {savingKpi?.l3Count ? formatCompactNumber(savingKpi.l3Count) : 0}
            <span className="global-kpi-name">L3 Category</span>
          </p>
        </div>

        <div className="global-kpi-box">
          <p className="global-kpi-num">
            {savingKpi?.l4Count ? formatCompactNumber(savingKpi.l4Count) : 0}
      ...
      <div className="option-bar">
        ...
        <Modal show={show} onHide={handleClose} centered>
          <Modal.Header className="form-header" closeButton>
            <Modal.Title className="form-head">Export Saving Data</Modal.Title>
          </Modal.Header>
          <Modal.Body>
            {isExporting ? (
