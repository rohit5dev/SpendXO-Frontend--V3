...
const HelpApproval = () => {
  ...
  const fetchHelpApprovalData = async () => {
    try {
      const response = await axios.get(
        `${apiUrls.urlPrefix}/help/help-query-by-id/${Id}`,
        requestHeader.json
      );
  ...
  const handleApproval = async (e, st) => {
    ...
    try {
      const response = await axios.put(
        `${apiUrls.urlPrefix}/approval-window/help-approval/${Id}?status=${st}&adminComment=${responseMsg}`,
        {},
        requestHeader.json
      );
      if (response.data.status) {
        setStatus(st);
        if (st === "Approved") toast.success(toastMessages.helpApproval);
        if (st === "Rejected") toast.success(toastMessages.helpRejection);
            ...
            <div>
              <div className="d-flex justify-content-between align-items-center mb-4">
                <div className="d-flex align-items-center">
                  ...
                  <h5 className="card-title mb-0">Help Approval Details</h5>
                </div>
                <span
                  className="badge"
                  style={{
              ...
              {[
                {
                  label: "Title",
                  value: approvalDetailsData?.Title || "N/A",
                },

                {
                  label: "Section",
                  value: approvalDetailsData?.Section || "N/A",
                },

                {
                  label: "Priority",
                  value: approvalDetailsData?.Priority || "N/A",
                },

                {
                  label: "Date",
                  value:
                    Moment(approvalDetailsData?.RequestDate).format(
                      "DD MMM YYYY"
                    ) || "N/A",
                },
                ...
                {
                  label: "Comment",
                  value: approvalDetailsData?.Comment || "No Comments",
                },
                {
                  label: "Edited By",
                  value: approvalDetailsData?.EditedBy || "N/A",
                },
                {
                  label: "Edited On",
                  value:
                ...
                {
                  label: "Comments",
                  value: approvalDetailsData?.Comments || "N/A",
                  multiline: true,
                },
              ].map((item, index) => (
              ...
              <div className="row mb-3">
                ...
                <div className="col-8  text-start">
                  {approvalDetailsData.Admin_Comment === "" &&
                  status === "Pending" ? (
                    <textarea
                    ...
                    <textarea
                      className="form-control"
                      value={approvalDetailsData.Admin_Comment || ""}
                      disabled
                      rows="3"
                    ></textarea>
                  )}
