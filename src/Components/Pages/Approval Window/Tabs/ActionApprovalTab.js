...
const ActionApprovalTab = () => {
  ...
  // Table Data
  const [filteredActionTreeData, setFilteredActionTreeData] = useState([]);
  ...
  const fetchActionTreeData = async () => {
    try {
      ...
      const response = await axios.get(
        `${apiUrls.urlPrefix}/approval-window/action-approval-data`,
        requestHeader.json
      );
      if (response?.data) {
        const resData = response?.data;
        setActionTreeData(resData?.result);
        setFilteredActionTreeData(resData?.result);
  ...
  const resetFilters = () => {
    ...
    setFilteredActionTreeData(actionTreeData);
  };
  ...
  useEffect(() => {
    const filteredData = actionTreeData.filter((row) => {
      const isWithinDateRange =
        (!startDate || moment(row.EditedOn).isSameOrAfter(startDate, "day")) &&
        (!endDate || moment(row.EditedOn).isSameOrBefore(endDate, "day"));
      return (
    ...
    setFilteredActionTreeData(filteredData);
  }, [section, approver, status, startDate, endDate, actionTreeData]);
    ...
    <div>
      ...
      >
        ...
        <table className="table table-striped" style={{ fontSize: rowFont }}>
          ...
          <tbody>
            {filteredActionTreeData.map((row) => (
              <tr key={row.id} style={{ borderBottom: "1px solid #ddd" }}>
                ...
                <td style={defaultStyles}>
                  {moment(row.EditedOn).format("DD MMM YYYY") || "N/A"}
                </td>
                <td style={defaultStyles}>
                  <span
                    onClick={() => {
                      navigate(`/approval-window/action-approval/${row.Id}`);
                    }}
...
export default ActionApprovalTab;
