...
const HelpApprovalTab = () => {
  ...
  const fetchHelpData = async () => {
    try {
      ...
      const response = await axios.get(
        `${apiUrls.urlPrefix}/approval-window/help-approval-data`,
        requestHeader.json
      );
      if (response?.data) {
        const resData = response?.data;
        setHelpData(resData?.result);
        setFilteredHelpData(resData?.result);
  ...
  const resetFilters = () => {
    ...
    setFilteredHelpData(helpData);
  };

  // Filter help desk data
  useEffect(() => {
    const filteredData = helpData.filter((row) => {
      const isWithinDateRange =
        (!startDate || moment(row.RequestDate).isSameOrAfter(startDate)) &&
        (!endDate || moment(row.RequestDate).isSameOrBefore(endDate));
      return (
    ...
    setFilteredHelpData(filteredData);
  }, [section, priority, status, startDate, endDate, helpData]);
            ...
            {filteredHelpData.map((row) => (
              <tr key={row.id} style={{ borderBottom: "1px solid #ddd" }}>
                ...
                <td style={defaultStyles}>
                  <span
                    onClick={() => {
                      navigate(`/approval-window/help-approval/${row.Id}`);
                    }}
